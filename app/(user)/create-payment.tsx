...
const CreatePayment = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const response = await axios.get(
        'https://quickbill-backlend.vercel.app/customers',
        {
          headers: {
            UserID: userId,
          },
  ...
  const handleCustomerSelect = async (customer: Customer) => {
    ...
    const customerCode = await AsyncStorage.setItem(
      'CustomerCode',
      customer.Code
    );
  ...
  const fetchBills = async () => {
    ...
    try {
      ...
      if (!userId || !companyId || !customerCode || !prefix) {
        Alert.alert(
          'Error',
          'Missing required data. Please ensure all fields are filled.'
        );
        return;
      }

      const response = await axios.get(
        'https://quickbill-backlend.vercel.app/api/payment-bills',
        {
          headers: {
            UserID: userId,
            CompanyID: companyId,
                ...
                <View style={styles.modalContent}>
                  ...
                  <View style={styles.modalBody}>
                    ...
                    <View style={styles.modalInfo}>
                      ...
                      <Text style={styles.modalValue}>
                        ₹{selectedBill.Balance.toLocaleString('en-IN')}
                      </Text>
                    </View>
  ...
  const BillSummary = () => {
    ...
    const totalAdjustedAmount = adjustedBills.reduce(
      (sum, bill) => sum + bill.adjustedAmount,
      0
    );
                  ...
                  >
                    {isLoadingBills ? (
                      ...
                      <>
                        ...
                        />
                        <Text style={styles.viewBillsButtonText}>
                  ...
                  {bills.length > 0 && (
                    <View style={styles.billsContainer}>
                      <View style={styles.billsHeader}>
                        <Text style={styles.billsHeaderTitle}>
                          Outstanding Bills
                        </Text>
